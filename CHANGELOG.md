...
## [1.3.2] — 2026-09-10
### Update Channels Revamp & Downloads Repack Location
- **Update Channel Revamp (`--latest` and `--beta`)**:
  - Added `--latest` flag to pull the absolute latest git tag instead of official release.
...
- **Repack Save to Downloads**:
  - `ricer repack` automatically saves repacked `.zip` archives inside the user's `~/Downloads` directory by default (`$XDG_DOWNLOAD_DIR`).
  - Automatically routes bare filenames (e.g. `ricer repack my-rice.zip`) to `~/Downloads/my-rice.zip`.
  - Automatically creates `~/Downloads` if not already present.
...
- **Repack Integration**:
  - `ricer repack` automatically captures `~/.config/systemd/user` configurations across Omarchy, Caelestia, Garuda, and Generic format profiles.
